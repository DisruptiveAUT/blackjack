	...
	while (1)
	{
		...
		else if (ESTADO == APOSTAR)
		{
			...
			if (pos_pantalla.px != 0 && tocadoPantalla == false)
			{
				if ((pos_pantalla.px >= 15 && pos_pantalla.px <= 47 &&
					 pos_pantalla.py >= 100 && pos_pantalla.py <= 132) &&
					dinero >= 1)
				{ // boton de anadir 1 ficha
					apuesta = apuesta + 1;
					dinero = dinero - 1;
					tocadoPantalla = true;
				}
		...
		else if (ESTADO == JUGAR)
		{
			...
			if (contadorBaraja < 4)
			{
				...
				for (i = 0; i < 2; i++)
				{ // las dos cartas obligatorias del Jugador
					manoJugador[i] = robarCarta(contadorBaraja, baraja);
					contadorBaraja++;
					contadorJugador++;
					if (manoJugador[i].valorNum == 11)
					{ // si es un as
						if (cartasJugador + manoJugador[i].valorNum > 21)
						{ // el as cuenta como 11 o 1 segun convenga
				...
				for (i = 0; i < 2; i++)
				{ // las dos cartas obligatorias del crupier
					manoCrupier[i] = robarCarta(contadorBaraja, baraja);
					contadorBaraja++;
					contadorCrupier++;
					if (manoCrupier[i].valorNum == 11)
					{
						if (cartasCrupier + manoCrupier[i].valorNum > 21)
						{ // el as cuenta como 11 o 1 segun convenga
			...
			if (((pos_pantalla.px >= 198 && pos_pantalla.px <= 246 &&
				  pos_pantalla.py >= 96 && pos_pantalla.py <= 128) &&
					 tocadoPantalla == false ||
				 (cartasJugador >= 21 && crupierMostradas == false)) ||
				contadorJugador >= 6)
			{ // si se decide ya jugar pulsando el boton stay o el jugador ya ha superado 21
				crupierMostradas = true;
				tocadoPantalla = true;
				while (cartasCrupier < 17)
				{ // el crupier deja de robar si tiene 17 o mas
					robarCartaCrupier();
				}
				...
				if (calcularPartida(cartasJugador, cartasCrupier) && devolverSeg() - segActual >= 4)
				{ // si se pierde la partida cambiar estado
					ESTADO = GANAR;
				...
				else if (!(calcularPartida(cartasJugador, cartasCrupier)) && devolverSeg() - segActual >= 4)
				{
					...
					// vaciarVariables();
					 ...
					 pos_pantalla.py >= 64 && pos_pantalla.py < 96 && tocadoPantalla == false)
			{ // pulsa el boton de hit
				tocadoPantalla = true;
				robarCartaJugador();
